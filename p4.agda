...
map-++ {A} {B} {f} {x ∷ xs} {ys} =
  ...
  ≡⟨ refl ⟩
    f x ∷ map f (xs ++ ys)
  ≡⟨ cong (λ l -> f x ∷ l) (map-++ {A} {B} {f} {xs} {ys}) ⟩
...
∈-++ : ∀ {A : Set} {z : A} {xs ys : List A}
       ...
       → (z ∈ xs) ⊎ (z ∈ ys)
∈-++ {A} {z} {[]} {ys} p = inj₂ p
∈-++ {A} {z} {x ∷ xs} {ys} (inj₁ p₁) = inj₁ (inj₁ p₁)                 -- z ≡ x
∈-++ {A} {z} {x ∷ xs} {ys} (inj₂ p₂) with ∈-++ {A} {z} {xs} {ys} p₂  -- z ̸≡ x
... | inj₁ r = inj₁ (inj₂ r)
... | inj₂ r = inj₂ r
...
∈-map {A} {B} {f} {z} {x ∷ xs} (inj₂ p₂) = inj₂ (∈-map {A} {B} {f} {z} {xs} p₂)
...
todos-iguales-map {A} {B} {f} {[]} p = tt
todos-iguales-map {A} {B} {f} {x ∷ []} p = tt
todos-iguales-map {A} {B} {f} {x ∷ (y ∷ ys)} p = cong f (proj₁ p) , todos-iguales-map {A} {B} {f} {y ∷ ys} (proj₂ p)
